...
from server_manager import ManagedServers, ensure_running, stop_server_on_port
...
with st.sidebar:
    ...
    # Log paths are used for diagnostics if startup times out
    servers = ManagedServers(
        ...
        # DeepSeek can take minutes to load; wait generously
        q5_start_timeout_s=900.0,
        q6_start_timeout_s=1200.0,
    )

    colA, colB = st.columns(2)
    with colA:
        if st.button("Start servers"):
            with st.status("Starting llama-servers…", expanded=True) as s:
                ok, msg = ensure_running(servers)
                if ok:
                    s.update(label="Servers running ✅", state="complete")
                    st.success(msg)
                else:
                    s.update(label="Failed to start ❌", state="error")
                    st.error(msg)
