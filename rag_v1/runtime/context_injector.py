...
def apply_rag_and_wiki_parallel(
    ...
    fast_base_url: str | None = None,
    fast_model_id: str | None = None,
) -> Tuple[List[Dict[str, Any]], list, list, "Optional[SearchEvidence]"]:
    """
    Run document RAG, wiki RAG, and (optionally) live web search concurrently,
    then inject all three into messages.

    The three fetches are independent and safe to run in parallel.
    ...
    """
    with ThreadPoolExecutor(max_workers=3, thread_name_prefix="rag_par") as pool:
    ...
    return out, rag_sources, wiki_images if wiki_ctx_block else [], search_evidence
